...
class PlayState extends MusicBeatState
{
	...
	//stupid fucking Kade Engine... Now my dialogue code on this exe is so damn convoluded...
	...
	//Extra Event Shitz
	var forceMustHit:Bool;
	var forceMustHitFalse:Bool;
	var forceGFSection:Bool;

	//Starvation Alley Variables
	var filthshit:BGSprite;
	//var charOverlay:FlxSprite;
	//var charOverlayResult:FlxSprite;
	var charColor:FlxColor;
	var sun:BGSprite;
	var sunTween:FlxTween;
	var skyFadeTween:FlxTween;
	var fadeHUDTween:FlxTween;
	var unfadeHUDTween:FlxTween;
	var skyone:BGSprite;
	public var cutsceneIllusion:Bool = false;
	...
	var daigleufggeeNAEMMEA = Paths.formatToSongPath(SONG.song);
	...
	if(dialogueList.contains(daigleufggeeNAEMMEA)){
			//I hate kade engine i hate kade engine
					...
					dialogue = CoolUtil.coolTextFile("assets/data/" + daigleufggeeNAEMMEA + "/cutscene.txt");
					usesDialogue = true;
					trace(dialogue);
				}
			...
			if(dialogueEndList.contains(daigleufggeeNAEMMEA)){
				...
				//	trace(dialogueEnd);
					...
					dialogueEnd = CoolUtil.coolTextFile("assets/data/" + daigleufggeeNAEMMEA + "/cutsceneEnd.txt");
		...
		switch (curStage)
		{
			...
			case 'alley': //Week 1
				...
				GameOverSubstate.characterName = 'pico-starve-dead';

				var skytwo:BGSprite = new BGSprite('alley/skyTwo', -400, -100, 0.1, 0.1);
				add(skytwo);

				skyone = new BGSprite('alley/skyOne', -400, -100, 0.1, 0.1);
				add(skyone);

				sun = new BGSprite('alley/sun', -50, 250, 0.15, 0.15);
				add(sun);
				
				var cityback:BGSprite = new BGSprite('alley/cityBack', -520, -200, 0.2, 0.2);
				cityback.setGraphicSize(Std.int(cityback.width * 0.85));
				cityback.updateHitbox();
				add(cityback);

				var cityfront:BGSprite = new BGSprite('alley/cityFront', -520, -200, 0.3, 0.3);
				cityfront.setGraphicSize(Std.int(cityfront.width * 0.85));
				cityfront.updateHitbox();
				add(cityfront);

				var extrawall:BGSprite = new BGSprite('alley/extrawall', -1150, -700);
				extrawall.setGraphicSize(Std.int(extrawall.width * 0.9));
				//add(extrawall);

				var alley:BGSprite = new BGSprite('alley/alleyWall', -850, -625);
				alley.setGraphicSize(Std.int(alley.width * 0.9));
				add(alley);
				
				var trashcan:BGSprite = new BGSprite('alley/trash', 200, -50);
				trashcan.setGraphicSize(Std.int(trashcan.width * 0.8));
				add(trashcan);

				filthshit = new BGSprite('alley/filth', 200, 370);
				//filthshit.setGraphicSize(Std.int(filthshit.width * 0.8));

/*				charOverlay = new FlxSprite(0, 0);
				charOverlay.loadGraphic(Paths.image('alley/overlay.png'));
				//charOverlay.blend = MULTIPLY;

				charOverlayResult = new FlxSprite(0, 0);
				FlxSpriteUtil.alphaMaskFlxSprite(dadGroup, charOverlay, charOverlayResult); */
		...
		switch(curStage)
		{
			...
			case 'alley':
				charColor = 0xffD487C2;
				//charColor.setRGB(213, 33, 103, 100);
				//charColor.alpha = 0.39;


				dad.color = charColor;
				boyfriend.color = charColor;
		}
...
/*		var file:String = Paths.json(songName + '/dialogue'); //Checks for json/Psych Engine dialogue
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			if (sunTween != null)
				sunTween.active = false;
			if (skyFadeTween != null)
				skyFadeTween.active = false;
			if (fadeHUDTween != null)
				fadeHUDTween.active = false;
			if (unfadeHUDTween != null)
				unfadeHUDTween.active = false;
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (sunTween != null)
				sunTween.active = true;
			if (skyFadeTween != null)
				skyFadeTween.active = true;
			if (fadeHUDTween != null)
				fadeHUDTween.active = true;
			if (unfadeHUDTween != null)
				unfadeHUDTween.active = true;
	...
	override public function update(elapsed:Float)
	{
		...
		if(botplayTxt.visible && !cutsceneIllusion) {
	...
	/*		case 'Advanced Alt Sing Animation':
				var char:Character = dad;
				switch(value1.toLowerCase()) {
					case 'gf' | 'girlfriend':
						char = gf;
				...
				if (char != null)
				{
					...
					char.idleSuffix = value2;
					var animToPlay:String = singAnimations[Std.int(Math.abs(daNote.noteData))] + value2;
					char.playAnim(animToPlay, true);

					if(char.hasMissAnimations)
					{
					var missAnimToPlay:String = singAnimations[Std.int(Math.abs(daNote.noteData))] + 'miss' + value2;
					char.playAnim(missAnimToPlay, true);
					}
			...
			case 'Bite':
				FlxG.sound.play(Paths.sound('bite'));
			...
			case 'Move Camera Early':
				var toggleCamera:Int = Std.parseInt(value1);
				if(Math.isNaN(toggleCamera)) toggleCamera = 0;
			...
			case 'Fade HUD':
				...
				cutsceneIllusion = true;
				
				fadeHUDTween = FlxTween.tween(camHUD, {alpha: 0}, 0.5, {
					ease: FlxEase.linear,
				});
			...
			case 'Unfade HUD':
				...
				unfadeHUDTween = FlxTween.tween(camHUD, {alpha: 1}, 0.5, {
					ease: FlxEase.linear,
				});
				cutsceneIllusion = false;
	...
	function moveCameraSection(?id:Int = 0):Void {
		...
		if (!SONG.notes[id].mustHitSection && forceMustHit != true)
		{
			moveCamera(true);
			forceMustHitFalse = false;
			callOnLuas('onMoveCamera', ['dad']);
		}
		else if (SONG.notes[id].mustHitSection && forceMustHitFalse != true)
		{
			moveCamera(false);
			forceMustHit = false;
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if(char != null && char.hasMissAnimations)
		{
			...
			if (SONG.notes[curSection] != null)
			{
				...
				else if (SONG.notes[curSection].secondAltAnim || daNote.noteType == 'Second Alt Animation') {
					daAlt = '-altTwo';
				}
			}
	/*		var daAlt = '';
			...
			else if(daNote.noteType == 'Second Alt Animation') 
			{
				daAlt = '-alttwo';
			} */
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			if (SONG.notes[curSection] != null)
			{
				...
				else if (SONG.notes[curSection].secondAltAnim || note.noteType == 'Second Alt Animation') {
					altAnim = '-altTwo';
				}
			}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			/*	if(note.noteType == 'Alt Animation') 
				...
				else if(note.noteType == 'Second Alt Animation')
				{
					daAlt = '-alttwo';
				} */
	...
	{
		sunTween = FlxTween.linearPath(sun, [FlxPoint.get(-50, 250), FlxPoint.get(-50, -150)], 2, true,
			{type: FlxTweenType.PERSIST, ease: FlxEase.quartOut});
		//sunTween.active = true;

		skyFadeTween = 
		FlxTween.tween(skyone, {alpha: 0}, 2, {
